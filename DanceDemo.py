...
class DanceDemo:
    """Demo class to apply source animation to the target using different generators."""
    def __init__(self, filename_src, typeOfGen=2):
        ...
        if self.target.skeCount() == 0:
            raise ValueError("No skeletons found in target video. Ensure the video is processed correctly.")

        if typeOfGen == 1:  # Nearest
            print("Generator: GenNeirest")
            self.generator = GenNeirest(self.target)
        elif typeOfGen == 2:  # VanillaNN for skeleton
            print("Generator: GenVanillaNN (Skeleton)")
            self.generator = GenVanillaNN(self.target, loadFromFile=True, optSkeOrImage=1)
        elif typeOfGen == 3:  # VanillaNN for image
            ...
            raise ValueError(f"Invalid generator type: {typeOfGen}")

    def draw(self):
        ske = Skeleton()
        image_err = np.zeros((256, 256, 3), dtype=np.uint8)
        image_err[:, :] = (0, 0, 255)  # Red error image
